...
description: Strategically review PRs without switching branches; switch to run tests/checks on a PR branch only when the review concludes it is complete + mergeable. Updates base branch strictly, updates PR summaries, merges when green, otherwise posts @copilot action plans.
---
...
- At that point you MUST proceed directly with `gh pr checkout <PR>` to execute tests/builds.
...
- “This PR appears complete and mergeable. Switching to PR branch using `gh pr checkout <PR_NUMBER>` to run tests/builds/checks.”

2. Switch branches immediately and continue verification.
