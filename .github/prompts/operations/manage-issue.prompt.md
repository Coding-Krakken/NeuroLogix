...
### Mandatory Comment Cadence (All Agents)

For every non-trivial issue, agents must add comments at these checkpoints:

1. **Start comment** — when work begins (branch + plan)
2. **Midpoint comment** — after first validated commit/checkpoint
3. **Finish/Handoff comment** — final status, evidence, and next owner

Each checkpoint comment must include:
...
# Example: structured checkpoint update
gh issue comment 42 --body "Checkpoint (midpoint):\n\n- Branch: feature/42-seo-foundation\n- Commit: <short-sha>\n- Git status: clean\n- Checks: lint ✅, typecheck ✅, targeted tests ✅\n- Next: implement slice S3"
```
