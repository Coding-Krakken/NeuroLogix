...
- Deployment method (<DEPLOYMENT_PLATFORM>, etc.)
...
3. Wait for <DEPLOYMENT_PLATFORM> auto-deploy
4. Verify deployment completes

### Option 4: DNS Rollback to <PAYMENT_PROVIDER> Online (~5min)

1. Access DNS provider
2. Change CNAME: <APPLICATION_DOMAIN> → [<ROLLBACK_TARGET_ENDPOINT>]
3. Wait for propagation (usually <5 min)
4. Verify <PAYMENT_PROVIDER> Online is serving
