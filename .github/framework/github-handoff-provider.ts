...
export class GitHubHandoffProvider {
  ...
  constructor(
    ...
    this.templateService = templateService || new CommentTemplateService(config.templatesDir);
  }
   ...
   */
  async readComments(context: WorkItemContext, limit: number = 25): Promise<string[]> {
   ...
   */
  async readIssue(issueNumber: number): Promise<{ title: string; body: string }> {
    if (this.config.dryRun) {
      return {
    ...
    try {
      ...
      throw new Error(`Failed to read issue #${issueNumber}: ${(error as Error).message}`);
    }
  }
   ...
   */
  async readPR(prNumber: number): Promise<{ title: string; body: string; state: string }> {
    if (this.config.dryRun) {
      return {
    ...
    try {
      ...
      throw new Error(`Failed to read PR #${prNumber}: ${(error as Error).message}`);
    }
  }
  ...
  async getCIRunUrl(prNumber: number): Promise<string | undefined> {
    ...
    try {
      const { stdout } = await execFileAsync(
        'gh',
        ['pr', 'checks', String(prNumber), '--json', 'detailsUrl', '--jq', '.[0].detailsUrl'],
        { windowsHide: true }
      );
   ...
   */
  private async renderHandoffComment(data: HandoffCommentData): Promise<string> {
    const variables = {
  ...
  private async postComment(
    ...
    const existing = await this.findExistingCommentByBody(context, target, number, body);
    if (existing) {
      logInfo(`♻️ Reusing existing ${target} comment for #${number}: ${existing.commentUrl}`);
      return existing;
    }
  ...
  private async postCommentViaApi(
    ...
    const commentId = posted.id ?? this.extractCommentId(commentUrl) ?? Number.NaN;
  ...
  private async findExistingCommentByBody(
    ...
    try {
      const { stdout } = await execFileAsync(
        'gh',
        [target, 'view', String(number), '--json', 'comments'],
        { windowsHide: true }
      );
...
export function createGitHubHandoffProvider(
  ...
  const dryRun = config.githubDryRun ?? process.env['SUBZERO_GITHUB_DRY_RUN'] === '1';
  const commentTarget =
    config.commentTarget ??
    (process.env['SUBZERO_GITHUB_COMMENT_TARGET'] as 'pr_preferred' | 'issue_only' | 'pr_only') ??
    'pr_preferred';
