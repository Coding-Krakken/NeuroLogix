...
function parseArgs(argv: string[]): RouterArgs {
  ...
  const eventPath =
    values.get("--event-path") ?? process.env["GITHUB_EVENT_PATH"];
...
function collectIndentedContinuation(
  lines: string[],
  startIndex: number,
): string {
  const values: string[] = [];
...
function collectHeadingContinuation(
  lines: string[],
  startIndex: number,
): string {
  const values: string[] = [];
...
function extractMetadataValue(body: string, label: string): string | undefined {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (boldMatch) {
      ...
      const continued = normalizeMetadataValue(
        collectIndentedContinuation(lines, index + 1),
      );
    ...
    if (headingPattern.test(line)) {
      const continued = normalizeMetadataValue(
        collectHeadingContinuation(lines, index + 1),
      );
      if (continued.length > 0) {
        return continued;
      }
...
function inferType(labels: string[], body: string): string {
  ...
  const fromBody = pickByBodyOrDefault(
    body,
    "Work Item Type",
    "feature",
  ).toLowerCase();
...
function main(): void {
  ...
  const missingSections = requiredSections.filter(
    (section) => !sectionExists(sections, section),
  );
  ...
  const severity =
    ...
    (normalizeByPattern(
      pickByBodyOrDefault(body, "Severity", "S2"),
      /(S[0-4])/i,
    ) ||
      "S2");
  const priority =
    ...
    (normalizeByPattern(
      pickByBodyOrDefault(body, "Priority", "P2"),
      /(P[0-3])/i,
    ) ||
      "P2");
  const risk =
    labels.find((label) =>
      ["low", "medium", "high", "critical"].includes(label),
    ) ?? normalizeEnumValue(pickByBodyOrDefault(body, "Risk", inferredRisk));
  const blastRadius = normalizeEnumValue(
    pickByBodyOrDefault(body, "Blast Radius", "single-team"),
  );
  const componentArea = normalizeEnumValue(
    pickByBodyOrDefault(body, "Component Area", "platform"),
  );
  ...
  merged = mergeRules(
    merged,
    matrix.overrides.byDataSensitivity[dataSensitivity],
  );
  merged = mergeRules(
    merged,
    matrix.overrides.byDeploymentSurface[deploymentSurface],
  );
  ...
  const missingMetadata = requiredMetadata.filter(
    (field) => !String(classification[field]).trim(),
  );
  ...
  if (
    !isAllowedValue(rolloutMethod, ["all-at-once", "flag", "ring", "canary"])
  ) {
    invalidMetadata.push("rolloutMethod");
  }
  if (
    !isAllowedValue(dataSensitivity, [
      "public",
      "internal",
      "confidential",
      "regulated",
