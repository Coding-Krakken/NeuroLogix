...
function assertRequiredSectionsInPrTemplate(prTemplateBody: string, result: CheckResult): void {
...
function assertIssueTemplatesDeterministicOnly(
  ...
  if (nonDeterministic.length > 0) {
    fail(`Non-deterministic issue templates found: ${nonDeterministic.join(', ')}`, result);
  }
...
function assertCodeownersConsistency(codeownersBody: string, result: CheckResult): void {
  ...
  if (duplicates.length > 0) {
    fail(`Duplicate CODEOWNERS entries found: ${duplicates.join(', ')}`, result);
  }
...
function main(): void {
  ...
  if (!existsSync(prTemplatePath)) {
    fail('Missing canonical PR template: .github/pull_request_template.md', result);
  }
  ...
  if (result.warnings.length > 0) {
    process.stdout.write(`Policy conformance warnings:\n- ${result.warnings.join('\n- ')}\n`);
  }
  ...
  if (result.failures.length > 0) {
    ...
    process.stdout.write('Advisory mode active: failures reported but not blocking.\n');
    return;
  }
