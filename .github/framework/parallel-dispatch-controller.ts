...
export class ParallelDispatchController {
  ...
  constructor(
    private readonly executeNode: (node: ScheduledNode, attempt: number) => Promise<void>,
        ...
        const runPromise = this.runAttempt(node, attempt, effectivePolicy)
          .then(async result => {
            ...
            if (attempt <= effectivePolicy.maxRetries) {
              ...
              statusByNodeId[node.nodeId] = 'queued';
              const delayBeforeMs =
                effectivePolicy.backoffMs * effectivePolicy.backoffMultiplier ** (attempt - 1);
              await this.sleep(delayBeforeMs);
              queue.push({ node, attempt: attempt + 1 });
              queue.sort((left, right) => this.compareQueueEntries(left, right));
              return;
            }

            statusByNodeId[node.nodeId] = result.status === 'timeout' ? 'timed_out' : 'failed';
  ...
  private resolveLimits(limits: Partial<DispatchConcurrencyLimits>): DispatchConcurrencyLimits {
    const caps = limits.perPriorityCaps ?? DEFAULT_LIMITS.perPriorityCaps;

    return {
      maxParallelAgents: limits.maxParallelAgents ?? DEFAULT_LIMITS.maxParallelAgents,
      perPriorityCaps: {
        P0: caps.P0,
        P1: caps.P1,
        P2: caps.P2,
  ...
  private resolvePolicy(policy: Partial<DispatchPolicy>): DispatchPolicy {
    return {
      attemptTimeoutMs: policy.attemptTimeoutMs ?? DEFAULT_POLICY.attemptTimeoutMs,
      ...
      failFastFutureWaves: policy.failFastFutureWaves ?? DEFAULT_POLICY.failFastFutureWaves,
    };
  }
  ...
  private findDispatchableIndex(
    ...
    const runningTotal =
      runningByPriority.P0 + runningByPriority.P1 + runningByPriority.P2 + runningByPriority.P3;
    ...
    for (let index = 0; index < queue.length; index += 1) {
      ...
      if (runningByPriority[entry.node.priority] >= limits.perPriorityCaps[entry.node.priority]) {
        continue;
      }
  ...
  private assertConcurrency(
    ...
    const runningTotal =
      runningByPriority.P0 + runningByPriority.P1 + runningByPriority.P2 + runningByPriority.P3;
    if (runningTotal > limits.maxParallelAgents) {
      throw new Error('Concurrency invariant violated: runningTotal exceeded maxParallelAgents');
    }
