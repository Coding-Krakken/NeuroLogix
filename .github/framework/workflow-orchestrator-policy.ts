...
import type { WorkflowEnforcementConfig, WorkflowPolicyDecision, WorkflowSnapshot } from './types';
...
export class WorkflowOrchestratorPolicy {
  ...
  evaluate(snapshot: WorkflowSnapshot): WorkflowPolicyDecision {
    ...
    if (!snapshot.issueNumber && !isTinyHotfix) {
      ...
      requiredActions.push('Create or link a GitHub issue before continuing implementation');
    }
    ...
    if (
      ...
      reasons.push('Current branch does not follow issue branch naming convention');
      requiredActions.push(
        `Create/switch to branch: feature/${snapshot.issueNumber}-short-slug or bugfix/${snapshot.issueNumber}-short-slug`
      );
    }

    if (!snapshot.acceptanceCriteriaReviewed || !snapshot.existingPrContextReviewed) {
    ...
    if (snapshot.minutesSinceLastCommit >= this.config.commitCheckpointMinutes) {
      requiredActions.push(
    ...
    if (
      ...
      requiredActions.push('Open pull request now; branch has diverged beyond PR threshold');
    }
  ...
  private branchMatchesIssue(branchName: string | undefined, issueNumber: number): boolean {
    if (!branchName) {
      return false;
    }
