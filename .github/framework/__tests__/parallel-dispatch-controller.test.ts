...
describe('ParallelDispatchController concurrency + dependency semantics', () => {
  it('runs independent nodes in parallel and keeps dependent node in later wave', async () => {
    ...
    const wave0 = await controller.executeWave(scheduler.toRuntimeWave(plan, 0), {
      taskId: 'TASK-B1',
      runId: 'RUN-B1',
    ...
    const wave2 = await controller.executeWave(scheduler.toRuntimeWave(plan, 2), {
      taskId: 'TASK-B1',
      runId: 'RUN-B1',
      schedulePlanHash: plan.schedulePlanHash,
    ...
    expect(started.slice(1, 3).sort((left, right) => left.localeCompare(right))).toEqual([
      'B',
      'C',
    ]);
