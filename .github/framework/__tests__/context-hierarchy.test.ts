...
const createConfig = (memoryHierarchyEnabled: boolean): HybridExecutionConfig => ({
  mode: 'hybrid',
  flags: {
  ...
  it('returns L1/L2/L3 slices with token estimates when enabled', () => {
    ...
    const result = hierarchy.compose({
      task: {
        ...
        acceptanceCriteria: ['Acyclic graph validation', 'Context slices + token estimates'],
      },
      l2Items: [
    ...
    expect(result.slices.map(slice => slice.tier)).toEqual(['L1', 'L2', 'L2', 'L3']);
    expect(result.slices.every(slice => slice.estimatedTokens > 0)).toBe(true);
  });
