...
describe('AgentDispatcher', () => {
  ...
  describe('dispatch', () => {
    ...
    it('should build prompt with handoff URL', async () => {
      const handoffUrl = 'https://github.com/owner/repo/pull/123#issuecomment-123456';
...
describe('postHandoffAndDispatch', () => {
  it('should enforce both handoff posting and dispatch', async () => {
    ...
    const mockHandoffProvider = {
      postHandoff: jest.fn().mockResolvedValue({
        commentUrl: 'https://github.com/owner/repo/pull/123#issuecomment-123456',
        commentId: 123456,
      }),
    };
    ...
    expect(mockHandoffProvider.postHandoff).toHaveBeenCalledWith(mockContext, mockHandoffData);
  });
  ...
  it('should throw if dispatch fails after handoff is posted', async () => {
    ...
    await expect(
      postHandoffAndDispatch(mockHandoffProvider, mockDispatcher, mockContext, mockHandoffData)
