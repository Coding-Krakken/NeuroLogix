...
describe('GitHubHandoffProvider', () => {
  ...
  beforeEach(() => {
    ...
    provider = new GitHubHandoffProvider({ dryRun: true, templatesDir }, 'pr_preferred');
    ...
    mockHandoffData = {
      ...
      status: 'Done',
      scopeCompleted: ['Implemented UI components', 'Added unit tests', 'Updated documentation'],
      keyDecisions: [
        {
      ...
      risks: ['New dependency added (React Query)', 'Breaking change in API interface'],
  ...
  describe('comment target resolution', () => {
    it('should use PR when commentTarget is pr_preferred and PR exists', async () => {
      ...
      const result = await providerPRPreferred.postHandoff(mockContext, mockHandoffData);
    ...
    it('should use Issue when commentTarget is issue_only', async () => {
      ...
      const result = await providerIssueOnly.postHandoff(mockContext, mockHandoffData);
    ...
    it('should throw when commentTarget is pr_only but no PR exists', async () => {
      ...
      await expect(providerPROnly.postHandoff(contextNoPR, mockHandoffData)).rejects.toThrow(
        'PR number required'
      );
    });
  });
