...
describe('WorkflowTelemetry', () => {
  it('tracks lifecycle metadata into final summary', () => {
    ...
    telemetry.trackIssueCreated(taskId, 42, 'https://github.com/Coding-Krakken/.subzero/issues/42');
    telemetry.trackBranchCreated(taskId, 'feature/42-github-workflow');
    telemetry.trackCommit(taskId, 'abc1234', 'feat(framework): add orchestration policy (#42)');
    telemetry.trackCommit(taskId, 'def5678', 'test(framework): add policy tests (#42)');
    telemetry.trackPrOpened(taskId, 100, 'https://github.com/Coding-Krakken/.subzero/pull/100');
