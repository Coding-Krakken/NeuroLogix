...
describe('ContextResolver', () => {
  ...
  describe('error handling', () => {
    it('should throw when repo cannot be resolved', async () => {
      jest.spyOn(resolver as any, 'detectRepo').mockResolvedValueOnce(undefined as never);
    ...
    it('should throw when branch name cannot be resolved', async () => {
      ...
      jest.spyOn(branchResolver as any, 'detectBranch').mockResolvedValue(undefined as never);
  ...
  describe('branch name pattern detection', () => {
    it('should extract issue number from feature branch', async () => {
      ...
      jest.spyOn(mockResolver as any, 'detectRepo').mockResolvedValue('owner/repo' as never);
    ...
    it('should return undefined for non-standard branch names', async () => {
      const mockResolver = new ContextResolver();
      jest.spyOn(mockResolver as any, 'detectBranch').mockResolvedValue('main' as never);
      jest.spyOn(mockResolver as any, 'detectRepo').mockResolvedValue('owner/repo' as never);

      await expect(mockResolver.resolve()).rejects.toThrow('Failed to resolve issue number');
    });
  });
});
