...
describe('ParallelDispatchController dispatch lifecycle events', () => {
  it('emits required metadata and deterministic sequence ordering', async () => {
    ...
    const run = async (): Promise<string> => {
      ...
      const starts = result.events.filter(event => event.event === 'hybrid.dispatch.start');
      const terminals = result.events.filter(event => event.event !== 'hybrid.dispatch.start');
      expect(starts.length).toBe(terminals.length);

      expect(telemetry.getHybridDispatchEvents('TASK-B4').length).toBe(result.events.length);
