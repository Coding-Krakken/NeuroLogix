...
const toEventLabel = (event: HandoffWorkflowEvent): string => {
  if (event.kind === 'dispatch') {
    ...
    const exit = event.exitCode !== undefined ? ` exit=${String(event.exitCode)}` : '';
...
export const buildHandoffWorkflowReportFromText = (text: string): HandoffWorkflowReport =>
  analyzeHandoffWorkflow(parseWorkflowLogText(text));

export const formatHandoffWorkflowReport = (report: HandoffWorkflowReport): string => {
  if (report.issues.length === 0) {
    return 'No issue-linked workflow events found.';
  }

  const lines: string[] = [`Tracked issues: ${String(report.issues.length)}`, ''];
