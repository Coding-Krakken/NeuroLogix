...
const DISPATCH_SUMMARY_PATTERN =
  /Dispatch command executed to\s+[`'"]?([a-z0-9-]+)[`'"]?/i;
const NEXT_TARGET_PATTERN =
  /Next routing target(?:\s+after[^:]+)?:\s*[`'"]?([a-z0-9-]+)[`'"]?/i;
const HANDOFF_ARTIFACT_PATTERN =
  /(\.github\/.handoffs\/[a-z0-9-]+\/handoff-[^\s`'"]+\.md)/i;
...
const parseHandoffUrl = (text: string): string | undefined =>
  text.match(HANDOFF_URL_PATTERN)?.[1];

const parseNextTarget = (text: string): string | undefined =>
  text.match(NEXT_TARGET_PATTERN)?.[1];
...
const collectGaps = (events: HandoffWorkflowEvent[]): string[] => {
  ...
  events.forEach((event, index) => {
    ...
    if (event.kind === "dispatch" && !event.handoffArtifact) {
      gaps.add(
        `Dispatch to ${event.targetAgent ?? "unknown"} missing handoff artifact.`,
      );
    }
    ...
    if (event.nextTargetAgent) {
      const hasFollowOnDispatch = events
        ...
        .some(
          (nextEvent) =>
            nextEvent.kind === "dispatch" &&
            nextEvent.targetAgent === event.nextTargetAgent,
        );
...
export const analyzeHandoffWorkflow = (
